...
static target_ulong disas_insn(DisasContext *s, target_ulong pc_start)
{
    ...
    if(pc_start == 0x804841b){
        printf("pc at 0x804841b\n");
    }
 ...
 next_byte:
    ...
    if(xt_enable_debug)
        if(s->pc < 0xc0000000 && s->pc > 0x1000000) // only for user space
...
#ifdef CONFIG_TCG_XTAINT
        if(xt_enable_size_mark && op == 6){ // only for push
            if(ot == OT_BYTE)
                gen_op_XT_mark(XT_SIZE_BEGIN, 8, 0);
            else if(ot == OT_WORD)
                gen_op_XT_mark(XT_SIZE_BEGIN, 16, 0);
...
//                curr_esp = cpu_single_env->regs[R_ESP];
//                gen_op_XT_mark(XT_INSN_CALL, cpu_regs[R_ESP], next_eip);
            }
#endif /* CONFIG_TCG_XTAINT */
            gen_eob(s);
            break;
...
#ifdef CONFIG_TCG_XTAINT
        if(xt_enable_size_mark && ot == OT_WORD)
            gen_op_XT_mark(XT_SIZE_BEGIN, 16, 0);
#endif /* CONFIG_TCG_XTAINT */
        gen_pop_T0(s);
    ...
    case 0x1bf: /* movswS Gv, Eb */
        {
            ...
            d_ot = dflag + OT_WORD;
#ifdef CONFIG_TCG_XTAINT
            if(xt_enable_size_mark && d_ot == OT_WORD){
                gen_op_XT_mark(XT_SIZE_BEGIN, 16, 0);
            }
#endif /* CONFIG_TCG_XTAINT */
...
//            curr_esp = cpu_single_env->regs[R_ESP];
//            gen_op_XT_mark(XT_INSN_RET, cpu_T[0], curr_esp);
//            gen_op_XT_mark(XT_INSN_RET, cpu_T[0], cpu_regs[R_ESP]);
        }
#endif /* CONFIG_TCG_XTAINT */
